        ...
        <main class="p-6 max-w-md mx-auto space-y-6">
            ...
            <?php if ($order): ?>
                ...
                <div class="bg-slate-800/80 backdrop-blur-md rounded-3xl border border-slate-700 p-6 shadow-xl relative overflow-hidden flex flex-col gap-8">
                    ...
                    <!-- Background glow -->
                    <div class="absolute top-0 right-0 w-48 h-48 bg-emerald-500/10 rounded-full blur-3xl -mr-10 -mt-10 pointer-events-none"></div>
                    ...
                    <div class="text-center relative z-10 pt-2 pb-4">
                        <div class="w-24 h-24 mx-auto bg-gradient-to-br <?= $order['status'] === 'cancelled' ? 'from-rose-500 to-red-600' : 'from-emerald-400 to-teal-500' ?> rounded-full flex items-center justify-center shadow-lg shadow-emerald-500/30 mb-5 border-4 border-slate-800">
                            <i class="fas <?= $currentIcon ?> text-4xl text-white"></i>
                        </div>
                        
                        <h2 class="font-extrabold text-2xl font-outfit <?= $order['status'] === 'cancelled' ? 'text-rose-400' : 'text-emerald-400' ?>">
                            <?= $currentStatusLabel ?>
                        </h2>
                    ...
                    <div class="relative z-10 pb-4">
                        <div class="flex justify-between items-center relative">
                            ...
                            ?>
                            <div class="absolute left-[10%] top-1/2 h-1 bg-emerald-500 -translate-y-1/2 z-0 rounded-full transition-all duration-700" style="width: calc(<?= $progressPercent ?>% * 0.8)"></div>
                            
                            <?php foreach ($statuses as $index => $status): 
                                $isPast = $index < $currentIndex;
                                $isActive = $index === $currentIndex;
                                
                                $bgColor = $isPast || $isActive ? 'bg-emerald-500 border-emerald-400' : 'bg-slate-800 border-slate-600';
                                $textColor = $isPast || $isActive ? 'text-white' : 'text-slate-500';
                                $glow = $isActive ? 'shadow-[0_0_15px_rgba(16,185,129,0.6)]' : '';
                            ?>
